...
class OutputTypeTestCase(ApiDBTestCase):
    ...
    def test_get_output_types(self):
        output_types = self.get("data/output_types")
        self.assertEquals(len(output_types), 3)

    def test_get_output_type(self):
        output_type = self.get_first("data/output_types")
        output_type_again = self.get("data/output_types/%s" % output_type["id"])
        self.assertEquals(output_type, output_type_again)
        self.get_404("data/output_types/%s" % fields.gen_uuid())
    ...
    def test_create_output_type(self):
        ...
        self.output_type = self.post("data/output_types", data)
    ...
    def test_update_output_type(self):
        ...
        self.put("data/output_types/%s" % output_type["id"], data)
        output_type_again = self.get(
            "data/output_types/%s" % output_type["id"])
        self.assertEquals(data["name"], output_type_again["name"])
        self.put_404("data/output_types/%s" % fields.gen_uuid(), data)

    def test_delete_output_type(self):
        output_types = self.get("data/output_types")
        self.assertEquals(len(output_types), 3)
        output_type = output_types[0]
        self.delete("data/output_types/%s" % output_type["id"])
        output_types = self.get("data/output_types")
        self.assertEquals(len(output_types), 2)
        self.delete_404("data/output_types/%s" % fields.gen_uuid())
